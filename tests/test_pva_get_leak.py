...
def measure_soak(pv_names: list[str], duration_s: int = 60,
    ...
    rss_series.append((0.0, _rss_mb()))
    iterations = 0

    if live_file:
        live_file.write(f"  [soak] starting — {duration_s}s, {len(pv_names)} PVs\n")
        live_file.flush()
        ...
        if now >= t_next_sample:
            ...
            if live_file:
                live_file.write(
                    ...
                    f"iters={iterations}  delta={rss - rss_series[0][1]:+.2f} MB\n"
                )
                live_file.flush()
    ...
    # Linear regression slope (MB/s) to detect slow creep
    n = len(rss_series)
    if n >= 2:
    ...
    return {
        ...
        "threshold_mb": 15.0,
        # fail if slope > 0.05 MB/s OR total delta > 15 MB
        "passed": delta < 15.0 and slope_mb_per_s < 0.05,
    }
